...
| 类别 | 现状 | 结论 |
|------|------|------|
| 美术图片 | 50 张 PNG 全部存在，代码引用的每个 key 均有对应文件（86 个单测全过，含 artmanifest 资源自检） | **无缺失**，主要是"可选升级重生成" |
| 孤儿图片 | `fx_freeze.png` 有图但代码未引用 | 属于**代码接线任务**，不是美术缺口 |
...
├── README.md                  # 本文件：总览与索引
├── art/
│   ├── art-inventory.md       # 现有 50 张图清单：key、用途、规格、引用位置
│   └── art-needs.md           # 待办美术需求：缺口、接线、可选升级项 + 即梦生成规格
└── audio/
