...
- 首页旧英文按钮残影：操作台由 548×350、Alpha 238 调整为 568×410、Alpha 255，并向上覆盖原图 `START GAME` 发光外框顶部；实机确认旧文字与轮廓不再透出。
- 角色介绍拥挤：卡片由 260px 扩为 360px，说明区由 252×92/10px 调整为 344×100/11px、17px 行高，四行被动与 Q/E/R 不再被过度缩小。
- 底部黄色说明遮挡：第二排整体上移，锁定提示移入卡片安全区并提升至 12px；1280×720 下不再与画布底边相切。
- 战斗英雄拉伸：待机呼吸从横纵反向缩放改为 0.6% 等比缩放，朝向翻转只改变 X 轴符号，不改变素材宽高比；移动时继续关闭呼吸缩放。
- 新增 `visualui.test.js`，固定首页遮罩范围、角色说明安全区和英雄等比缩放三项回归门禁。
